...
sigma = 10  # Smoothing window in time steps
...
for neuron in range(network.num_neurons):
    plt.eventplot(network_spike_times_list[neuron], linelengths=0.8, label=f"Neuron {neuron}")

# network spikes
plt.xlabel("Time (ms)")
plt.ylabel("Network Neuron")
plt.title("Raster Plot of Network Neuron Responses")
plt.legend()
plt.show()
